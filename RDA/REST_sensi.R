...
valid_sensitivity_modes <- c("cutoff_layer_4", "c_sigma2_vec_1", "sigma_mu2_1")
...
file_name <- "REST.K562.DNase.counts.mat.rds"
label_name <- "REST.K562.sites.chip.labels.tss.dist.rds"
...
if (identical(sensitivity_mode, "cutoff_layer_4")) {
  sampler_args$cutoff_layer <- 4L
...
cat("REST sensitivity analysis complete\n")
cat("Task id:", task_id, "\n")
