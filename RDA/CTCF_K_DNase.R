...
file_name_K_new = "CTCF.K562.DNase.counts.mat.rds"
label_K_new = "CTCF.K562.sites.chip.labels.rds"
...
#   FUNcluster = kmeans,
#   K.max      = 10,
#   B          = 100,            # number of Monte Carlo draws (≥ 50 is common)
#   verbose    = FALSE,          # suppress intermediate output
#   nstart     = 25              # additional argument passed to kmeans(…, nstart=25)
# )
# plot(gap_km, 
#      main = "Gap‐statistic for k‐means", 
#      xlab = "Number of clusters k", 
#      ylab = "Gap(k)")
# 
# # 3) Extract the numeric table
# gap_km$Tab
# 
# best_k_km <- maxSE(
#   f      = gap_km$Tab[, "gap"],
#   SE.f   = gap_km$Tab[, "SE.sim"],
...
cutoff_layer <- 2; print(paste("num of cor nodes",2^(cutoff_layer+1)-1))    # Layer for correlated nodes cutoff
warm_start <- 0
burnin <- warm_start + 20  # Burn-in period
total_iter <- burnin + 10;print(total_iter)  # Total iterations
c_sigma2_vec <- 100  # Hyperparameter for sigma^2 vector. 1/c is the mean
sigma_mu2 <- 0.01     # Hyperparameter for sigma_mu^2
...
saveRDS(Z_cortree,file.path(data_path,"cortree_CTCF_K_DNase.rds"))
...
# # summarize ---------------------------------------------------------------
# 
# Z_cortree = readRDS(file.path(data_path,"cortree_CTCF_K_DNase.rds"))
# 
# y_upper_lim = 15
# par(mfrow = c(2,2))
# plot_pos_neg_profiles(X, chip_labels,
...
# # combine into a data.frame
# ari_table <- data.frame(
#   Method = c("K-means", "PAM", "Cortree", "Initial"),
#   ARI    = c(ari_kmeans, ari_pam, ari_cortree, ari_initial)
# )
# 
# knitr::kable(ari_table, digits = 3,
#              caption = "Adjusted Rand Index by clustering method")
...
# p1 <- hist_by_group_gg(chip_data,Z_kmeans, title = "Kmeans",y_max = max_log_count)
# p2 <- hist_by_group_gg(chip_data,Z_pam, title = "PAM",y_max = max_log_count)
# p3 <- hist_by_group_gg(chip_data,Z_cortree, title = "Cor-tree",y_max = max_log_count)
# 
# # 
# # combined <- (p0+p1 )/( p2 + p3)   # or: p1 + p2 + p3 + p4 + plot_layout(ncol = 2)
# # print(combined)
...
# m1 = cluster_mean_gg(X,Z_kmeans, title = "Kmeans",y_max=y_upper_lim)
# m2 = cluster_mean_gg(X,Z_pam, title = "PAM",y_max=y_upper_lim)
# m3 = cluster_mean_gg(X,Z_cortree, title = "Cor-tree",y_max=y_upper_lim)
# 
# # combined <- (m0+m1 )/( m2 + m3)   # or: p1 + p2 + p3 + p4 + plot_layout(ncol = 2)
# # print(combined)
# 
